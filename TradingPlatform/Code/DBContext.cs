...
namespace TradingPlatform.Model {
    ...
    public static class DataProvider {
        ...
        public static List<DealTypeDropDown> GetDealTypes()
        {
            if (HttpContext.Current.Session["DealTypes"] == null)
                HttpContext.Current.Session["Dealtypes"] = GenerateDropDown();
            return HttpContext.Current.Session["DealTypes"] as List<DealTypeDropDown>;
        }
        public static List<Counters> GetCountersDropDown()
        {
            if (HttpContext.Current.Session["Counters"] == null)
                HttpContext.Current.Session["Counters"] = GenerateCounterDropDown();
            return HttpContext.Current.Session["Counters"] as List<Counters>;
        }
        ...
        static List<DealTypeDropDown> GenerateDropDown()
        {
            List<DealTypeDropDown> DealDrop = new List<DealTypeDropDown>();
            SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["GSEQConnectionString"].ConnectionString);
 
            conn.Open();
          
            SqlCommand cmd = new SqlCommand("select ID,DealType from tblDealTypes with(nolock)", conn);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    DealDrop.Add(new DealTypeDropDown
                    {
                        DealTypeID = Convert.ToInt32(dr["ID"]),
        ...
        static List<Counters> GenerateCounterDropDown()
        {
            List<Counters> CounterDrop = new List<Counters>();
            SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["GSEQConnectionString"].ConnectionString);
           
            conn.Open();

            SqlCommand cmd = new SqlCommand("select ID,[Short Name] as CounterName  from tblCounters with(nolock)", conn);
            SqlDataAdapter mda = new SqlDataAdapter(cmd);
            DataSet mds = new DataSet("ds");
            mda.Fill(mds);
            ...
            if (mdt.Rows.Count > 0)
            {
                ...
                {
                    CounterDrop.Add(new Counters
                    {
                        CounterID = Convert.ToInt32(mdr["ID"]),
                        CounterName = mdr["CounterName"].ToString()
                    }
                     );
                }
